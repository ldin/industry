...
@if(!empty($row->id))
	...
	@if(!empty($images))
		@foreach($images as $image)
			...
			<div class="" id="image-{{ $image->id }}">
				{{ Form::open(array('url' => 'admin/item-image/'.$row->id.'/'.$image->id, 'class' => 'form-group', 'files' => true)) }}
		            <div class="row">
	...
	<div class="row bg-info" id="image-add">
		...
		<div class="col-xs-12">
			...
			{{ Form::open(array('url' => 'admin/item-image/'.$row->id, 'class' => 'form-group', 'files' => true)) }}
	            <div class="row">
		            <div class="form-group col-sm-8 col-xs-12">
		                {{ Form::label('inputTextIMG', 'Описание') }}
